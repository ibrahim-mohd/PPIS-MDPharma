...
def extract_ligands(sorted_merged_dict, out_path, top_N=10, top_Ngraphs=20):
    ...
    for graph_index, key in enumerate(sorted_merged_dict.keys()):
        ...
        for index, (zinc_id, value) in tqdm (enumerate(sorted_merged_dict[key].items()), desc=f"Processing Graph {key}" ):
            ...
            u = mda.Universe(value["tpr"], value["gro"])
            chainA = u.atoms.fragments[0]
            chainA.chainIDs = ["A"] * len(chainA.chainIDs)
            chainB = u.atoms.fragments[1]
            chainB.chainIDs = ["B"] * len(chainB.chainIDs)
            protein_pdb_path = os.path.join(ligand_path, "protein.pdb")
            mda.Merge(chainA, chainB).atoms.write(protein_pdb_path, bonds=None)
