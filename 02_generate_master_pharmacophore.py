...
def print_summary(data, path):
    """Print the summary of total number of features, types and chain IDs etc for the provided thresholds
       and write it to a file named 'summary_master_pharma.dat' in the given path."""
    
    # Prepare output file path
    output_file = os.path.abspath(path) + "/summary_master_pharma.dat"
    
    lines = []  # Collect lines to write to file

    total_features = len(data['points'])
    lines.append(f"\nTotal number of features: {total_features}")
    print(lines[-1])  # Print to console
...
def main():
    ...
    if possible_hydrophobic_sites:
        ...
        for resid in possible_hydrophobic_sites:
            ...
            # The following fails if Glycine is identified as a hydrophobic site
            # in such cases we simply continue with the next residue
    ...
    ## output the summary of features
    print ( os.path.dirname (args.out_file))
